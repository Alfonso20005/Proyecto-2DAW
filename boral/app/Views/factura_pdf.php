...
<head>
    ...
    <title>Factura - <?= esc($mes) ?>/<?= esc($anio) ?></title>
    <style>
        /* General */
...
<div class="container">
   ...
   <div class="header" style="position: relative; height: 100px;">
    <h1 style="position: absolute; top: 20px; left: 20px; font-size: 48px; color: #28a745; margin: 0;">FACTURA</h1>
    ...
    <table class="info-table">
        <tr>
            <td class="info-left" style="width: 48%;">
                ...
                <p><strong>Nombre:</strong> Boral Pasteleria S.L</p>
                <p><strong>Dirección:</strong> Polígono Argualas, 20</p>
                <p><strong>Teléfono:</strong> +34 669 71 01 07</p>
                <p><strong>Email:</strong> <?= esc($empresa['email']) ?></p>
            </td>
            <td class="info-separator" style="width: 20%;"></td>
            <td class="info-right" style="width: 32%;">
                <div class="info-title">Datos del Distribuidor</div>
                <p><strong>Razón Social:</strong> <?= esc($distribuidor['razon_social']) ?></p>
                <p><strong>Nombre:</strong> <?= esc($distribuidor['nombre']) ?> <?= esc($distribuidor['apellidos']) ?></p>
                
                <?php 
                        $telefono_formateado = preg_replace('/(\d{3})(\d{2})(\d{2})(\d{2})/', '+34 $1 $2 $3 $4', esc($distribuidor['telefono']));
                    ?>
                <p><strong>Teléfono:</strong> <?= $telefono_formateado ?></p>
    ...
    <!-- Factura -->
     ...
     MES DE <?= esc($mesTexto) ?> de <?= esc($anio) ?>
</h2>
    ...
    <table class="invoice-table">
        ...
        <tfoot>
            <tr>
                <td colspan="4" class="total">Total de Pedidos:</td>
                <td class="total"><?= number_format($totalPedidos, 2, ',', '.') ?> €</td>
            </tr>
        </tfoot>
    </table>
</div>
