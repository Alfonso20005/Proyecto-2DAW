...
<title>Boral | Distribuidores</title>
<div class="row">
    <div class="col-12">

        <!-- Titulo con estilo de Bootstrap -->
        <p class="display-4 text-center text-primary font-weight-bold">DISTRIBUIDORES</p>

        <div class="d-flex justify-content-between align-items-center mb-3">
        
            <?php if (session()->get('role') !== 'Distribuidor'){ ?>
                <div>
                    <a href="<?php echo baseUrl();?>/distribuidores/nuevo" class="btn btn-success">
                        <i class="fa-solid fa-plus"></i> Nuevo Distribuidor
                    </a>
                </div>
            
                <div>
                    <a href="<?php echo baseUrl();?>/distribuidores/exportar" class="btn btn-success" id="exportar" data-bs-toggle="tooltip" 
                data-bs-placement="top" title="Excel"><i class="fa-regular fa-file-excel"></i></a>
                    &nbsp;
                    <a href="<?php echo baseUrl();?>/distribuidores/exportarPDF" class="btn btn-danger" id="pdf" data-bs-toggle="tooltip" 
         ...
         if(count($distribuidores)>0){
             foreach($distribuidores as $r){
                ...
                <tr>
                    ...
                    <td>
                        <a href="<?php echo baseUrl();?>/distribuidores/editar?id=<?php echo $r["id"];?>" class="btn btn-primary">
                        ...
                        &nbsp;&nbsp;  
                        <a href="<?php echo baseUrl();?>/distribuidores/imprimir?id=<?php echo $r["id"];?>" class="btn btn-dark">
   ...
   $(".borrar").click(function(){
           ...
           const swalWithBootstrapButtons = Swal.mixin({
                        ...
                        swalWithBootstrapButtons.fire({
                          title: "Desea eliminar al distribuidor?",
                          ...
                          if (result.isConfirmed) {
                              ...
                              $.ajax({
                                     data:{id:id},
                                     method:"POST",
                                     url: "<?php echo baseUrl();?>/distribuidores/eliminar", 
                                     success: function(result){
                                         if(result==1){
                                            swalWithBootstrapButtons.fire({
                                              title: "Eliminado!",
                                              text: "Distribuidor dado de baja",
                                              icon: "success"
                                            });
                                            padre.hide();
                                         }else{
                                             swalWithBootstrapButtons.fire({
                                              title: "No Eliminado!",
                                              text: "Distribuidor NO dado de baja",
                                              icon: "error"
                                            });
                                         }
                                    }
